...
function Header (props) {
	...
	const makeList = ((menu) => {
		...
		let items = menu.map((m, key) => {
			return <ListItemLink to={m.path} primary={m.text} icon={null} key={key.toString()} styles={styles}></ListItemLink>
		});
		console.log(items);
		let tempList =
	...
	let show = 
		<div className="rafuip-header">
			...
			<Drawer open={isOpen} className={{paper: classes.paper}} onClose={toggleMenu(false)}>
				<div
					tabIndex={0}
					role="button"
