...
function SelectColumns(props) {
  const {name, value, separator, style, label, lib, table,asArray, onChange,  eProps, sharedProps} = props;
  ...
  const {source, store, session} = appEnv;
  ...
  useEffect(() => {
    ...
    if (table == null || table.trim().length === 0 || lib == null || lib.trim().length === 0) {
      ...
      getTableColumns(store, session, source, lib, table)
        .then ( r => {
          setList(r);
          setSel([]);
          })
