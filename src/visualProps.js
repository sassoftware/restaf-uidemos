...
function visualProps() {
  ...
  const _makeProps = (modifiers, component, label) => {
    ...
    v.label = label;
    v.grow = (modifiers.grow === 'h') ? 'horizontal' : 'both';
  ...
  let items = {
    ...
    Inputarea: _makeProps({ value: ' ', type: 'char', style: "{eProps: {minRows: 2}}"}, 'Inputarea', 'Inputarea'),
    ...
    Checkbox: _makeProps({ value: 1, type: 'double' }, 'Checkbox', 'Checkbox'),
    ...
    DropdownMenu: _makeProps({grow: 'h', value: ' ', type: 'char', dataProps:  "{items: ['item 1', 'Item 2']}"}, 'DropdownMenu', 'Dropdown Menu'),
